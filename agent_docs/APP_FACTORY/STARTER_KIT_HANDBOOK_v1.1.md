...
## 🔧 v1.1 Addendum — Phase 5 Build/Prerender Doctrine (added during v3 build, 2026-06-09)

Two corrections surfaced during the Phase 5 cold-build proof and recorded here as permanent doctrine. They sharpen the v3 build contract.

### N4 — Client-component navs MUST defer `createClient`

A `"use client"` component still runs its render body during the **server-side render (SSR) pass** of static prerendering. If the component body contains:

```tsx
const Navbar = () => {
  const supabase = createClient();   // ❌ FIRES DURING PRERENDER SSR
  // ...
};
```

...then a cold `next build` with no `.env` will fail at prerender of any statically prerendered route that includes this component (e.g. a public marketing layout). The supabase client throws "URL and API key are required" before the route's HTML can be emitted.

**Correct pattern:** create the client inside `useEffect` so it only runs after mount on the client:
...
const Navbar = () => {
  ...
  useEffect(() => {
    const supabase = createClient();   // ✅ deferred to client-only
    // subscribe + fetch + cleanup
  }, []);
};
```

Apply this to ALL client-component navs in v3 by default. Build-safe is the default, not the optimization.

### C1 ledger correction — real predicate

C1's v2 ledger entry described the breaker as **"`/demo` creates a Supabase client at static prerender."** That phrasing was specific to the one site Run 001 hit. The **real predicate** is broader:

> *No `createClient` (server OR client) at prerender/SSR time, anywhere in the prerendered call graph.*

This includes:
- A `page.tsx` directly calling `createClient` (the v2 `/demo` case — fixed by deletion).
- A `"use client"` component called from a prerendered layout/page calling `createClient` at body-render time (the v3 NavbarHome case — fixed by Option A force-dynamic on the page + Option B useEffect-defer on the surviving navs).

The right grep predicate at any future audit:

```bash
# Step 1: server-side createClient in any prerendered route
grep -rnE "createClient" src/app | grep -iE "page|layout"

# Step 2: client-side createClient at body-render in any imported client component
grep -rn "createClient" src/components | grep -v "useEffect"
# inspect each hit — any createClient called at the top level of a "use client" component
# body is a prerender-time hazard if that component is rendered in a prerendered route.
```

Both grids must come back clean for C1 to be honestly closed.
...
### Fixed Nav Surface (Earned Addition — Token-Driven)

> 🔧 **ADDED (Phase 6, 2026-06-09):** v3 ships a **fixed-color nav surface** as a paired token. Some brand assets — a white-text logo, a fixed-color brand mark — cannot adapt per mode and need an invariant surface. Using `bg-secondary` would adapt to the active theme and leave white text on a pale panel in light mode (unreadable). Using `bg-primary` would invert across modes and break the dark mode the design system already balances. The right answer is an explicit invariant token.

```
bg-nav-bg              /* fixed dark nav surface; same value in :root and .dark */
text-nav-foreground    /* white text optimised for the fixed dark surface */
```

Rules:
- `--nav-bg` / `--nav-foreground` are **invariant** — same value in light and dark. This is the point.
- Use for nav/header chrome that hosts a brand asset that cannot adapt per mode.
- Do NOT use for general surfaces that should follow the theme (those stay `bg-secondary`, `bg-card`, etc.).
- Opacity modifiers (`bg-nav-bg/90`, `text-nav-foreground/70`) work — paired-token form, opacity-capable.
