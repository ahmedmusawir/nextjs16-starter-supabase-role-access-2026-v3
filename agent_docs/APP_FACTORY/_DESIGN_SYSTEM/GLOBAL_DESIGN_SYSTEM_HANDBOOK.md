...
**Earned addition — role identity tokens** (Starter Kit v3 RBAC harness):

`--role-superadmin` · `--role-admin` · `--role-member` (each with `-foreground` for surface usage).

**Earned addition — fixed nav surface** (Starter Kit v3, 2026-06-09):

`--nav-bg` · `--nav-foreground` — invariant across `:root` and `.dark`. Use this only for nav/header chrome hosting a brand asset (e.g. a white-text logo) that cannot adapt per mode. General surfaces stay `bg-secondary`/`bg-card`.
