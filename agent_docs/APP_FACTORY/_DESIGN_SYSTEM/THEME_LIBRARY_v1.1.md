...
| Nav bg | **invariant fixed dark surface** — same value across all four modes |
| Nav foreground | white (paired with nav bg for brand-asset readability) |
| Typeface | Saira |
| Radius | `0` (Metro flat) |
...
> **Nav tokens (added 2026-06-09):** `--nav-bg` / `--nav-foreground` are **invariant** across `:root`, `.dark`, `theme-bright`, and `dark.theme-deep`. They exist precisely so brand assets that cannot adapt per mode (a white-text logo, a fixed brand mark) read correctly in every theme. Override per family only if the brand mark itself changes per family — otherwise inherit the family-shared dark value. Neutral default in v3: `--nav-bg: 221 15% 24%` (#343a45), `--nav-foreground: 0 0% 100%`.
