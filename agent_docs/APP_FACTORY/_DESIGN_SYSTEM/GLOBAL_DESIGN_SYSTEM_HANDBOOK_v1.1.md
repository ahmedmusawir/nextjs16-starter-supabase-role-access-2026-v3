...
**Earned addition — fixed nav surface** *(added 2026-06-09; the kit "earns" this because some brand assets — a white-text logo, a fixed-color mark — cannot adapt per mode and need an invariant surface):*

`--nav-bg` · `--nav-foreground`

`--nav-bg` is **invariant** — identical value in `:root` and `.dark`. That's the whole point: it lets a white-text logo read in light mode without inverting the dark mode you already balanced. Do NOT use this for general adaptive surfaces (those stay `bg-secondary`, `bg-card`, etc.).
